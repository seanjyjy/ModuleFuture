...
import { StyleSheet, View, Text, Platform, TouchableOpacity, Modal} from 'react-native';
...
const DetailsCollection = () => {
    ...
    const nonFilledButton = (props) => (
        <TouchableOpacity  activeOpacity= {0.95} style={styles.shape} onPress = {() => change(1,props)}>
    ...
    const FilledButton = (props) => (
        <TouchableOpacity  activeOpacity= {0.95} style={styles.newShape} onPress = {() => change(2,props)}>
    ...
    const below = () => (
        <SignInButton func = {() => switchToAddInfo()}>
            <Text style={{...globalFontStyles.OSSB_17, color: 'white'}}>Continue</Text>
        </SignInButton>
    )
