...
const Login = () => {
    ...
    const [rightBar, setRightBar] = useState({...styles.horizontalLine, left: 48, backgroundColor: 'white'});
        ...
        frame : (
            <View style={{flex :1}}>
                <View style={{flexDirection:'column', flex :2, bottom: 60}}>
                    <View style ={{flex : 3, justifyContent:'center', alignItems:'center', bottom: 30}}>
                        <Text style ={{...globalFontStyles.NB_28, color: '#7F8E9E'}}>Login via NUSnet ID ?</Text>
                    </View>
                    <View style={ {flex : 6, top : Platform.OS === 'android' ? 40 : 0}}>
                        <View style = {{flex : 1, flexDirection: 'row',justifyContent:'space-evenly'}}>
                            <YesNoButton func = {() => pressingYesOrNo("No")}>
                                <Text style={{...globalFontStyles.OSSB_17, color: 'white'}}>No</Text>
                            </YesNoButton>
                            <YesNoButton func = {() => pressingYesOrNo("Yes")}>
                                <Text style={{...globalFontStyles.OSSB_17, color: 'white'}}>Yes</Text>
                            </YesNoButton>
                        </View>
                    </View>
                </View>
    ...
    const pressingSignUp = (value) => {
        if (value === 1) {
            ...
            setLeftBar({...styles.horizontalLine, right: 53, backgroundColor: 'white'})
            setRightBar({...styles.horizontalLine, left: 48, backgroundColor: '#FC5185'})
            setBottomText({
            ...
            setLeftBar({...styles.horizontalLine, right: 53, backgroundColor: '#FC5185'})
            setRightBar({...styles.horizontalLine, left: 48, backgroundColor: 'white'})
            setBottomText({
                firstText: 'Don\'t have an account?',
                secondText: 'Sign up'
            });
    ...
    return (
            <Background>
                ...
                <View style={styles.textHeight}>
                    ...
                    <Text
                        onPress = {() => {pressingSignUp(originalMiddle.key)}}
                        style={{...globalFontStyles.OSR_15, marginLeft: 5, color: '#fb5581', paddingRight: 5}}
                    >
