...
import { View, StyleSheet, Dimensions, Platform,Text, FlatList, TouchableOpacity} from "react-native";
...
const ContentPage = () => {
    ...
    const semButton = (sem) => {
        let state = true;
        return (
            <TouchableOpacity style={{...styles.touchStyle, marginVertical: 20}} activeOpacity = {0.9} onPress={() => navigation.navigate(sem.toString())}>
                <View style={{...styles.viewStyle}}>
                    {sideColor(state)}
                    <View style={{width: '42%', height: '100%', justifyContent: "center", alignItems: 'center'}}>
    ...
    const sideColor = (state) => {
        return (
            <View style={{flexDirection: 'row'}}>
                <View style={{width: 60, height: 0.08 * height, borderRadius: 20,
                ...
                <View style={{width: 40, height: 0.08 * height,
                    backgroundColor: (state ? '#FF6161' : '#61FF86'),
                    right: 40,
                    elevation: 2,
                    shadowColor: "#000",
    ...
    return (
        <View style={{ flex: 1 , alignItems: 'center'}}>
            ...
            <Page year={"Content Page"}>
                <View style={{width: '100%', height: '97%', justifyContent:'center', alignItems:'center'}}>
                    <FlatList
                        data={menu}
